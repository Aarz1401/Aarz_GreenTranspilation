...
if __name__ == "__main__":
    ...
    tests = []
    tests.append(["@.a"])  # simple linear, one key
    tests.append(["@..aA..b"])  # two keys with a lock
    tests.append(["@#a"])  # unreachable key
    tests.append(["@..aA", "..B#.", "....b"])  # multi-row, multiple keys/locks
    tests.append(["@A..b"])  # lock without its key makes key unreachable
    tests.append(["@..", ".#aA", "..b"])  # jagged rows
    tests.append(["@....", "abcde", "ABCDE", "....."])  # many keys in a row
    tests.append(["@..#..a", "..#....", "##.##.#", "..b.A.."])  # maze with walls and locks
    tests.append(["@Aa"])  # lock blocks the only path to its key
    tests.append(["@."])  # no keys
