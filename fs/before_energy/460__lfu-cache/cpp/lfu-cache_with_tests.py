...
def scenario_example(cache_class):
    c = cache_class(2)
    s = 0
    c.put(1, 1)
    c.put(2, 2)
...
def scenario_capacity_zero(cache_class):
    c = cache_class(0)
    c.put(1, 1)
    s = 0
    s += c.get(1)      # -1
...
def scenario_updates(cache_class):
    c = cache_class(2)
    s = 0
    c.put(1, 10)
    c.put(2, 20)
    c.put(1, 11)       # update value and freq
    c.put(3, 30)       # evict 2
    s += c.get(2)      # -1
    s += c.get(1)      # 11
    s += c.get(3)      # 30
...
def scenario_lru_tie(cache_class):
    c = cache_class(2)
    s = 0
    c.put(1, 1)
    c.put(2, 2)
    c.put(3, 3)        # evict 1 (LRU among freq=1)
    s += c.get(1)      # -1
    s += c.get(2)      # 2
    s += c.get(3)      # 3
    return s

def scenario_complex(cache_class):
    c = cache_class(3)
    s = 0
    c.put(1, 1)
    c.put(2, 2)
    ...
    s += c.get(1)      # 1 -> f2
    s += c.get(2)      # 2 -> f2
    c.put(4, 4)        # evict 3 (min freq=1)
    s += c.get(3)      # -1
    s += c.get(4)      # 4 -> f2
    s += c.get(1)      # 1 -> f3
    c.put(5, 5)        # evict 2 (LRU among f2: 2 vs 4)
    s += c.get(2)      # -1
    s += c.get(4)      # 4
    s += c.get(5)      # 5
    return s
...
def run_all_scenarios():
    total = 0
    for scenario in (scenario_example,
                     scenario_capacity_zero,
                     scenario_updates,
                     scenario_lru_tie,
                     scenario_complex):
        s1 = scenario(LFUCache)
        s2 = scenario(LFUCache2)
        assert s1 == s2
        total += s1
    return total
