...
if __name__ == "__main__":
    # Test cases replicated from the C++ benchmark
    tests = []

    # 1) Basic overlapping and non-overlapping
    tests.append([[1, 2], [2, 3], [6, 1]])

    # 2) Non-overlapping sequence
    tests.append([[0, 1], [2, 1], [4, 1], [6, 1], [8, 1], [10, 1]])

    # 3) Same position, increasing sizes
    tests.append([[0, 100], [0, 1], [0, 2], [0, 3], [0, 4]])

    # 4) Chain overlaps
    tests.append([[0, 2], [1, 2], [2, 2], [3, 2], [4, 2], [5, 2], [6, 2], [7, 2]])

    # 5) Nested intervals and extensions
    tests.append([[1, 5], [2, 3], [3, 1], [2, 5], [1, 2], [0, 6]])

    # 6) Large coordinates, safe sums
    tests.append([[1000000000 - 100, 50], [1000000000 - 150, 75], [500000000, 100], [1000000000 - 200, 60]])

    # 7) Mixed random-like pattern
    tests.append([[5, 4], [13, 2], [2, 3], [11, 5], [7, 1], [6, 4], [15, 2], [14, 3]])

    # 8) Back-and-forth placements
    tests.append([[10, 3], [7, 2], [12, 4], [9, 1], [8, 5], [11, 2], [6, 3]])
    ...
    t10 = []
    for i in range(60):
        left = i - (0 if i % 3 == 0 else 1)
        side = (i % 7) + 1
        t10.append([left, side])
    tests.append(t10)

    # Benchmark runner (expects a Solution class with fallingSquares method to be defined)
    checksum = 0
    ...
    for iter in range(iterations):
        for i in range(len(tests)):
            res = Solution().fallingSquares(tests[i])
            for v in res:
                checksum += v
            checksum ^= (len(res) * (i + 1))
