...
if __name__ == "__main__":
    ...
    tests = [
        ...
        ("FFFFFFFF", "a"),
        ("TT", "ababa"),
        ("TFFTTFTFTFTTFTFFFTFTTTFTFF", "aababa")
    ]
    ...
    for iter in xrange(iterations):
        for tc in tests:
            res = sol.generateString(tc[0], tc[1])
            size = len(res)
            mix = (size + 0x9e3779b97f4a7c15 + ((checksum << 6) & mask) + ((checksum >> 2) & mask)) & mask
            checksum = (checksum ^ mix) & mask
            if size > 0:
                checksum = (checksum + ord(res[0])) & mask
                checksum = (checksum + ord(res[-1])) & mask
            else:
