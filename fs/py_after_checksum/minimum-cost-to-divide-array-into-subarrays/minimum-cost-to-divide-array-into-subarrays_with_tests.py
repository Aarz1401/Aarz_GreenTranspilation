...
if __name__ == "__main__":
    ...
    def uniform_int(rng, a, b):
        ...
        span = hi - lo + 1
        if span <= 0:
            # should not happen for given ranges
            return lo
    ...
    # 1) Single element
    tests.append({'nums': [5], 'cost': [3], 'k': 10})

    # 2) Two elements, small values, k = 0
    tests.append({'nums': [1, 2], 'cost': [2, 1], 'k': 0})

    # 3) All equal nums and costs
    tests.append({'nums': [7] * 10, 'cost': [7] * 10, 'k': 5})
    ...
    for i in xrange(n):
        nums[i] = i + 1
    tests.append({'nums': nums, 'cost': cost, 'k': 3})
    ...
    for i in xrange(n):
        nums[i] = n - i
        cost[i] = i + 1
    tests.append({'nums': nums, 'cost': cost, 'k': 2})
    ...
    rng = MT19937(123456)
    for i in xrange(n):
        nums[i] = uniform_int(rng, 0, 1000)
        cost[i] = uniform_int(rng, 1, 100)
    tests.append({'nums': nums, 'cost': cost, 'k': 7})
    ...
    cost = [0] * n
    for i in xrange(n):
        cost[i] = 1000000 - (i % 1000)
    tests.append({'nums': nums, 'cost': cost, 'k': 1000})
    ...
    cost = [0] * n
    for i in xrange(n):
        nums[i] = 0 if (i % 2 == 0) else 1000000
        cost[i] = 1 if (i % 2 == 0) else 1000
    tests.append({'nums': nums, 'cost': cost, 'k': 50})
    ...
    for i in xrange(n):
        nums[i] = uniform_int(rng, 0, 1000)
        cost[i] = 100 if (i % 10 == 0) else 0
    tests.append({'nums': nums, 'cost': cost, 'k': 5})
    ...
    for i in xrange(n):
        nums[i] = (i * 1L * i) % 1000
        cost[i] = (i * 7) % 300 + 1
    tests.append({'nums': nums, 'cost': cost, 'k': 17})
    ...
    for iter in xrange(iterations):
        for i in xrange(len(tests)):
            t = tests[i]
            checksum += (sol.minimumCost(t['nums'], t['cost'], t['k']) ^ (iter + i))
    end = time.time()
