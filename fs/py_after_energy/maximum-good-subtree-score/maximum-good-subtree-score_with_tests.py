...
if __name__ == "__main__":
    ...
    tests = []
    tests.append({'vals': [7], 'par': [0]})                # single digit
    tests.append({'vals': [0], 'par': [0]})                # zero value
    tests.append({'vals': [11], 'par': [0]})               # repeated digit (invalid mask)
    tests.append({'vals': [98], 'par': [0]})               # two unique digits
    tests.append({'vals': [101], 'par': [0]})              # repeated digit (invalid mask)
    tests.append({'vals': [1234], 'par': [0]})             # multiple unique digits
    tests.append({'vals': [1000000000], 'par': [0]})       # many repeated zeros (invalid mask)
    tests.append({'vals': [24680], 'par': [0]})            # even digits including zero
    tests.append({'vals': [13579], 'par': [0]})            # odd digits
    tests.append({'vals': [987654321], 'par': [0]})        # long unique digits
    ...
    for iter in xrange(iterations):
        for tc in tests:
            checksum += sol.goodSubtreeSum(tc['vals'], tc['par'])

    elapsed = int((time.time() - start) * 1000000.0)
