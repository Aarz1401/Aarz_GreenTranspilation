...
if __name__ == "__main__":
    ...
    # Test 1
    tests.append(TestCase([5], [3], 2))

    # Test 2
    tests.append(TestCase([1, 2], [3, 4], 5))

    # Test 3
    tests.append(TestCase([3, 1, 4, 1, 5], [2, 7, 1, 8, 2], 3))
    ...
    for i in range(1, 9):
        nums.append(10 * i)
        cost.append(i)
    tests.append(TestCase(nums, cost, 4))

    # Test 5: uniform nums, patterned cost
    nums = [1] * 10
    cost = []
    for i in range(10):
        v = 2 if i % 3 == 0 else (5 if i % 3 == 1 else 3)
        cost.append(v)
    tests.append(TestCase(nums, cost, 1))
    ...
    for i in range(12, 0, -1):
        cost.append((i * 5) % 9 + 1)
    tests.append(TestCase(nums, cost, 7))

    # Test 7: larger nums
    nums = []
    cost = []
    for i in range(1, 16):
        nums.append(i * 100 + (i % 7))
        cost.append(16 - i)
    tests.append(TestCase(nums, cost, 10))

    # Test 8: quadratic pattern nums, modular cost
    nums = []
    cost = []
    for i in range(1, 19):
        nums.append((i * i) % 50 + 1)
        cost.append((i * 7) % 10 + 1)
    tests.append(TestCase(nums, cost, 6))
    ...
    for i in range(1, 21):
        cost.append((i % 5) + 1)
    tests.append(TestCase(nums, cost, 8))

    # Test 10: alternating pattern
    nums = []
    cost = []
    for i in range(1, 26):
        v = 100 if i % 4 == 0 else (30 if i % 4 == 1 else (60 if i % 4 == 2 else 10))
        nums.append(v)
        cost.append((i % 7) + 1)
    tests.append(TestCase(nums, cost, 9))
    ...
    for _ in range(iterations):
        #checksum = 0
        for i in range(10):
            res = sol.minimumCost(tests[i].nums, tests[i].cost, tests[i].k)
        #     checksum += int(res)
        # sink = checksum
