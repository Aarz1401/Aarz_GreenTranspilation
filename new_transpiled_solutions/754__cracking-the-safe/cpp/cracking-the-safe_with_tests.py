...
if __name__ == "__main__":
    ...
    for it in xrange(iterations):
        for n, k in tests:
            ...
            for ch in res:
                if not isinstance(ch, (int, long)):
                    ch = ord(ch)
                h ^= ch
                h &= mask
                h = (h * 1099511628211) & mask
            mix = (len(res) + 0x9e3779b97f4a7c15 + ((h << 6) & mask) + (h >> 2)) & mask
            h ^= mix
            h &= mask
            term = (it * 0x9e3779b97f4a7c15) & mask
            checksum ^= (h ^ term) & mask
            checksum &= mask

    elapsed_ms = (time.time() - start) * 1000.0
    print "Checksum: %d TotalLength: %d Elapsed(ms): %f" % (checksum, total_len, elapsed_ms)