...
def main():
    ...
    test_cases = [
        ...
        [1, 1],  # 3) two identical
        [1, 2],  # 4) two distinct
        [3, 3, 3, 3, 3],  # 5) all same
        [1, 2, 1, 2, 1, 2, 1],  # 6) alternating
        [1, 2, 3, 4, 5, 6, 7],  # 7) increasing distinct
        [1000000000, -1000000000, 0, 1000000000, -1000000000],  # 8) extremes and zeros
        [5, 5, 6, 6, 5, 6, 7, 7, 8, 8],  # 9) multiple pairs
        [9, 9, 1, 2, 3, 3, 2, 1, 4, 4, 5, 6, 7, 7, 7],  # 10) mixed with repetitions
    ]
    ...
    for test in test_cases:
        if isinstance(test, tuple):
            sol.subsequencesWithMiddleMode(*test)
        else:
            sol.subsequencesWithMiddleMode(test)
    ...
    for _ in xrange(iterations):
        ...
        for test in test_cases:
            if isinstance(test, tuple):
                result = sol.subsequencesWithMiddleMode(*test)
            else:
                result = sol.subsequencesWithMiddleMode(test)
